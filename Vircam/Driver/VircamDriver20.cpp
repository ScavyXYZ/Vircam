...
#define CaptureSourceName L"Vircam"

//Use separate GUIDs for 64bit and 32bit so both can be installed at the same time
#ifdef _WIN64
// {390D7440-E6DA-4DA6-9143-531E8FEA5FFE}
DEFINE_GUID(CLSID_VircamCaptureService, 0x390d7440, 0xe6da, 0x4da6, 0x91, 0x43, 0x53, 0x1e, 0x8f, 0xea, 0x5f, 0xfe);
// {14F90186-AC28-4053-8154-F0D55FEC0F78}
DEFINE_GUID(CLSID_VircamCaptureProperties, 0x14f90186, 0xac28, 0x4053, 0x81, 0x54, 0xf0, 0xd5, 0x5f, 0xec, 0xf, 0x78);
#else
// {14602899-5DA2-40D0-8028-DDC012E2842E}
DEFINE_GUID(CLSID_VircamCaptureService, 0x14602899, 0x5da2, 0x40d0, 0x80, 0x28, 0xdd, 0xc0, 0x12, 0xe2, 0x84, 0x2e);
// {8D88A12A-4DDA-4463-86B9-3F2D5004179A}
DEFINE_GUID(CLSID_VircamCaptureProperties, 0x8d88a12a, 0x4dda, 0x4463, 0x86, 0xb9, 0x3f, 0x2d, 0x50, 0x4, 0x17, 0x9a);
#endif
...
enum EErrorDrawCase { EDC_ResolutionMismatch, EDC_NotConnected, EDC_Disconnected, _EDC_MAX };
...
private:
	HRESULT FillBuffer(IMediaSample* pSamp) override
	{
		...
		case SharedImageMemory::RECEIVERES_CAPTUREINACTIVE: {
			FillErrorPattern(ErrorDrawModes[EDC_NotConnected], &State, m_llFrame);
		...
		case SharedImageMemory::RECEIVERES_OLDFRAME: {
			...
			FillErrorPattern(ErrorDrawModes[EDC_Disconnected], &State, m_llFrame);
			break;
		}
		}
...
#undef RGBAF16toBGRAU8
	...
	static void ProcessImage(int InWidth, int InHeight, int InStride, SharedImageMemory::EFormat Format, SharedImageMemory::EResizeMode ResizeMode, SharedImageMemory::EMirrorMode MirrorMode, int Timeout, uint8_t* InBuf, ProcessState* State)
	{
		...
		{
			FillErrorPattern(ErrorDrawModes[EDC_ResolutionMismatch], State);
			return;
		}
	...
	static void FillErrorPattern(EErrorDrawMode edm, ProcessState* State, LONGLONG FrameNumber = -1)
	{
		//clear the buffer first
		size_t bufferSize = (size_t)State->BufWidth * State->BufHeight * State->BufBPP;
		ZeroMemory(State->Buf, bufferSize);
		if (edm == EDM_BLACK) return;
		if (FrameNumber >= 0 && FrameNumber < 5) return;

		//error text
		const char* errorText = "Unknown Error";
		switch (edm)
		{
		case EDM_GREENKEY:     errorText = "Disconnected"; break;
		case EDM_GREENYELLOW:  errorText = "Not Connected"; break;
		case EDM_BLUEPINK:     errorText = "Resolution Mismatch"; break;
		}

		//rendering text using GDI
		HDC hdc = CreateCompatibleDC(NULL);
		BITMAPINFO bmi = { 0 };
		bmi.bmiHeader.biSize = sizeof(BITMAPINFOHEADER);
		bmi.bmiHeader.biWidth = State->BufWidth;
		bmi.bmiHeader.biHeight = State->BufHeight;
		bmi.bmiHeader.biPlanes = 1;
		bmi.bmiHeader.biBitCount = State->BufBPP * 8;
		...
		HFONT hFont = CreateFontA(State->BufHeight / 15, 0, 0, 0, FW_BOLD, FALSE, FALSE, FALSE,
			DEFAULT_CHARSET, OUT_OUTLINE_PRECIS, CLIP_DEFAULT_PRECIS,
			CLEARTYPE_QUALITY, VARIABLE_PITCH, "Arial");
		SelectObject(hdc, hFont);

		RECT textRect = { 0, 0, 0, 0 };
		DrawTextA(hdc, errorText, -1, &textRect, DT_CALCRECT);
		int textHeight = textRect.bottom - textRect.top;
		int textWidth = textRect.right - textRect.left;

		//center
		int distance = 10;
		int totalBlockHeight = textHeight * 2 + distance; // two lines + distance
		int centerY = State->BufHeight / 2;

		//bot mirrored coord
		int normalTextTop = centerY - totalBlockHeight / 2;
		int normalTextBottom = normalTextTop + textHeight;

		//mirrored coord
		int mirroredTextTop = normalTextBottom + distance; // +distance
		int mirroredTextBottom = mirroredTextTop + textHeight;
		...
		DrawTextA(hdc, errorText, -1, &rectMain, DT_CENTER | DT_VCENTER | DT_SINGLELINE);
		...
		DrawTextA(hdc, errorText, -1, &rectMirrored, DT_CENTER | DT_VCENTER | DT_SINGLELINE);

		SetWorldTransform(hdc, &xformOld);

		memcpy(State->Buf, pBits, bufferSize);

		if (State->BufBPP == 4)
		{
			BYTE* p = State->Buf, * pEnd = State->Buf + bufferSize;
			for (; p < pEnd; p += 4) p[3] = 0xFF;
		}
	...
	STDMETHODIMP GetStreamCaps(int iIndex, AM_MEDIA_TYPE** ppmt, BYTE* pSCC) override
	{
		...
		*ppmt = CreateMediaType(&mt);
		...
		pCaps->MinFrameInterval = 10000000 / 240;
		pCaps->MaxFrameInterval = 10000000 / 10;
...
#pragma pack(4)
		...
		SetDlgItemTextW(hwnd, 1002, L"Not connected:");
		SetDlgItemTextW(hwnd, 1004, L"Disconnected:");
	...
	static INT_PTR CALLBACK MyDialogProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
	{
		...
		if (uMsg == WM_COMMAND)
		{
			...
			if (ItemID == 1003 && SubCommand == 1) ErrorDrawModes[EDC_NotConnected] = (EErrorDrawMode)SelectionIndex;
			if (ItemID == 1005 && SubCommand == 1) ErrorDrawModes[EDC_Disconnected] = (EErrorDrawMode)SelectionIndex;
...
private:
	...
	CCaptureSource(LPUNKNOWN lpunk, HRESULT* phr) : CSource("Source", lpunk, CLSID_VircamCaptureService, phr) {}
	...
	STDMETHODIMP GetPages(CAUUID* pPages) override
	{
		...
		*(pPages->pElems) = CLSID_VircamCaptureProperties;
		return NOERROR;
	}
};
...
__inline static GUID GetCLSIDVircamCaptureServiceNum(int i)
{
	GUID NumCLSID = CLSID_VircamCaptureService;
...
{
	if (IsEqualCLSID(rClsID, CLSID_VircamCaptureProperties)) return 1;
	if (memcmp(&rClsID, &CLSID_VircamCaptureService, sizeof(GUID) - 1)) return 0;
	unsigned char LastByteOffset = (rClsID.Data4[7] - CLSID_VircamCaptureService.Data4[7]);
...
static HRESULT RegisterFilters(BOOL bRegister)
{
	...
	{
		char* CapNumParam = strstr(GetCommandLineA(), "/i:VircamCaptureDevices=");
		if (CapNumParam) MaxCapNum = atoi(CapNumParam + sizeof("/i:VircamCaptureDevices=") - 1);

		const wchar_t* CapNameParam = wcsstr(GetCommandLineW(), L"/i:VircamCaptureName=");
		if (CapNameParam)
		{
			const wchar_t* CapNameStart = CapNameParam + sizeof("/i:VircamCaptureName=") - 1;
		...
		for (int i = 0; SUCCEEDED(hr) && i != MaxCapNum; i++)
			hr = AMovieSetupRegisterServer(GetCLSIDVircamCaptureServiceNum(i), GetCaptureSourceNameNum(pCaptureSourceName, i).str, achFileName, L"Both", L"InprocServer32");
		if (SUCCEEDED(hr)) hr = AMovieSetupRegisterServer(CLSID_VircamCaptureProperties, CaptureSourceName L" Configuration", achFileName, L"Both", L"InprocServer32");
	...
	if (SUCCEEDED(hr))
	{
		...
		if (SUCCEEDED(hr))
		{
			if (bRegister)
			{
				...
				for (int i = 0; SUCCEEDED(hr) && i != MaxCapNum; i++)
				{
					hr = fm->RegisterFilter(GetCLSIDVircamCaptureServiceNum(i), GetCaptureSourceNameNum(pCaptureSourceName, i).str, 0, &CLSID_VideoInputDeviceCategory, NULL, &rf2);
					...
					StringFromCLSID(GetCLSIDVircamCaptureServiceNum(i), &CLSID_Filter_Str);
			...
			{
				for (int i = 0; SUCCEEDED(hr) && i != MaxCapNum; i++)
					hr = fm->UnregisterFilter(&CLSID_VideoInputDeviceCategory, 0, GetCLSIDVircamCaptureServiceNum(i));
	...
	{
		for (int i = 0; SUCCEEDED(hr) && i != MaxCapNum; i++)
			hr = AMovieSetupUnregisterServer(GetCLSIDVircamCaptureServiceNum(i));
		if (SUCCEEDED(hr)) hr = AMovieSetupUnregisterServer(CLSID_VircamCaptureProperties);
